...
`~/.local` (ou `~/.npm-global` pour Copilot), `~/.cache` et `~/.copilot` (Copilot uniquement — session, jeton d'auth) restent, eux, des **volumes Podman nommés** (jamais des bind-mounts) : leur stockage vit entièrement à l'intérieur du disque de la VM `podman machine`, sans traverser virtiofs/9p — ils ne sont pas concernés par le risque virtiofs/9p ci-dessus.

Ils restent en revanche concernés par un risque UID/GID distinct, **confirmé sur matériel réel** : ces répertoires sont chownés à `devuser` (UID `1000`, codé en dur dans `workspace-base/Dockerfile`) au moment du build. Sur un host Linux dont l'utilisateur est lui-même UID `1000`, ça passe inaperçu. Sur macOS (UID `501`/`502` typiquement), `--userns=keep-id` ajoute bien l'UID hôte comme membre secondaire du groupe `devuser`, mais ça ne suffit à écrire que si le répertoire est aussi en écriture groupe — d'où `workspace-base/scripts/prepare-state-dir.sh`, qui pose systématiquement `chmod 2775` (pas un simple `chown`) sur ces répertoires. Symptôme observé avant ce correctif : `npm install -g` échouait avec `EACCES ... Your cache folder contains root-owned files` sur `~/.cache/npm`, alors qu'aucun fichier n'était réellement possédé par root — juste par un UID `1000` sans droit d'écriture groupe.
