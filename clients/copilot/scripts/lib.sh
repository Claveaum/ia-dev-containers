...
# et ensure_network_and_ip sont génériques (aucune référence à copilot) :
# partagés avec clients/mistral-vibe via scripts/common.sh, pas dupliqués ici.
...
# (pas un simple cache) : scopé par projet, pour qu'un paquet compromis
# installé dans un projet ne devienne pas silencieusement importable depuis
# un autre. ~/.cache ne contient que le cache de téléchargement npm (rien
# d'exécutable "installé") : partagé entre projets par simplicité, pour
# éviter de retélécharger les mêmes paquets pour chaque projet.
NPM_GLOBAL_VOLUME="copilot-npm-global-${PROJECT_NAME}"
